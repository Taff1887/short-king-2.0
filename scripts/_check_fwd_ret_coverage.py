"""Trace fwd_ret_1m coverage by date in the monthly IS panel."""
...
cov = (
    df.groupby("Date")
      .agg(n_rows=("Ticker", "size"),
           n_fwd=("fwd_ret_1m", lambda s: s.notna().sum()))
      .reset_index()
)
cov["pct"] = 100 * cov["n_fwd"] / cov["n_rows"]
print(f"Total monthly dates: {len(cov)}")
print(f"Dates with 0 fwd_ret_1m: {(cov['n_fwd'] == 0).sum()}")
print(f"Dates with < 50 fwd_ret_1m: {(cov['n_fwd'] < 50).sum()}")
print()
